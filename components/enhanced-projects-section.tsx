...
const projects: Project[] = [
  ...
  },
  {
    ...
    description:
      "A forward-simulating optimizer for a one-shot, 200-pick dynasty startup draft, where the objective is expected end-state roster value rather than player rank. Players are priced by dyn_vor — availability × Σ γ^y · max(0, projected_y − replacement) over a four-year horizon — with replacement level calibrated per game rather than per season, and split into two levels because a starting slot and a bench slot compete against different alternatives; collapsing them to one prices 463 of 558 skill players at exactly zero. Age curves are fit over 20 seasons of nflverse weekly data as separate survival and retention hazards, shrunk toward a pooled positional prior on thin age×position strata, anchored at each position’s production peak to isolate aging from regression to the mean, and forced non-increasing as a post-condition. Candidates are then scored by Monte Carlo rollout under common random numbers — identical draw counts per candidate make the comparison exactly paired, and across 160 full drafts an unpaired stream discards 42% of that variance reduction — against seven synthetic managers whose pick noise follows σ = 0.315·pick^0.782, with survival probabilities from the normal CDF under continuity correction. League scoring reproduces Sleeper’s published PPR totals to 2e-15 across 558 players; the hot loop runs on flat NumPy arrays to stay inside a 60-second clock. In a paired A/B it beats naive best-available by +270 lineup points on 6/6 seeds — a result that validates the search, not the valuation. 307 tests, including property-based optimizer invariants, run hermetically offline.",
    techStack: [
      "Python",
      "NumPy",
      "pandas",
